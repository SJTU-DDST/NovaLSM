...
// Special Env used to delay background operations.
    ...
    public:
        ...
        Status NewWritableFile(const std::string &f, WritableFile **r) {
            ...
            public:
                DataFile(SpecialEnv *env, WritableFile *base) : env_(env),
                                                                base_(base) {}
            ...
            public:
                ManifestFile(SpecialEnv *env, WritableFile *b) : env_(env),
                                                                 base_(b) {}
    ...
    public:
        ...
        std::string
        Get(const std::string &k, const Snapshot *snapshot = nullptr) {
            ReadOptions options;
        ...
        void
        FillLevels(const std::string &smallest, const std::string &largest) {
    ...
    TEST(DBTest, GetMemUsage) {
        do {
            ...
            std::string val;
            ASSERT_TRUE(
                    db_->GetProperty("leveldb.approximate-memory-usage", &val));
    ...
    TEST(DBTest, GetSnapshot) {
        do {
            ...
            for (int i = 0; i < 2; i++) {
                std::string key = (i == 0) ? std::string("foo") : std::string(
                        200, 'x');
                ASSERT_OK(Put(key, "v1"));
    ...
    TEST(DBTest, GetEncountersEmptyLevel) {
        do {
            ...
            while (NumTableFilesAtLevel(0) == 0 ||
                ...
                ASSERT_LE(compaction_count, 100)
...
// if the database is shutdown during the memtable compaction.
    ...
    TEST(DBTest, RepeatedWritesToSameKey) {
        ...
        const int kMaxFiles =
                config::kNumLevels + config::kL0_StopWritesTrigger;
    ...
    TEST(DBTest, ApproximateSizes) {
        do {
            ...
            for (int run = 0; run < 3; run++) {
                ...
                for (int compact_start = 0;
                     compact_start < N; compact_start += 10) {
                    for (int i = 0; i < N; i += 10) {
                        ASSERT_TRUE(Between(Size("", Key(i)), S1 * i, S2 * i));
                        ASSERT_TRUE(Between(Size("", Key(i) + ".suffix"),
                                            S1 * (i + 1),
                                            S2 * (i + 1)));
                        ASSERT_TRUE(Between(Size(Key(i), Key(i + 10)), S1 * 10,
                                            S2 * 10));
                    }
    ...
    TEST(DBTest, ComparatorCheck) {
        ...
        public:
            const char *
            Name() const override { return "leveldb.NewComparator"; }
    ...
    TEST(DBTest, CustomComparator) {
        ...
        public:
            const char *
            Name() const override { return "test.NumberComparator"; }
        ...
        private:
            static int ToNumber(const Slice &x) {
                ...
                int val;
                char ignored;
                ASSERT_TRUE(sscanf(x.ToString().c_str(), "[%i]%c", &val,
                                   &ignored) == 1)
                        << EscapeString(x);
                return val;
            }
...
// Multi-threaded test:
        ...
        static void MTThreadBody(void *arg) {
            ...
            while (!t->state->stop.load(std::memory_order_acquire)) {
                ...
                if (rnd.OneIn(2)) {
                    ...
                    ASSERT_OK(db->Put(WriteOptions(), Slice(keybuf),
                                      Slice(valbuf)));
                } else {
                    ...
                    if (s.IsNotFound()) {
                        ...
                        ASSERT_LE(c, t->state->counter[w].load(
                                std::memory_order_acquire));
                    }
                }
                counter++;
    ...
    public:
        ...
        Status
        Put(const WriteOptions &o, const Slice &k, const Slice &v) override {
            return DB::Put(o, k, v);
        }
        ...
        void
        GetApproximateSizes(const Range *r, int n, uint64_t *sizes) override {
            for (int i = 0; i < n; i++) {
                sizes[i] = 0;
            }
        }
    ...
    static bool CompareIterators(int step, DB *model, DB *db,
        ...
        for (miter->SeekToFirst(), dbiter->SeekToFirst();
             ok && miter->Valid() &&
             dbiter->Valid(); miter->Next(), dbiter->Next()) {
            count++;
        ...
        do {
            ...
            for (int step = 0; step < N; step++) {
                ...
                if (p < 45) {  // Put
                    k = RandomKey(&rnd);
                    v = RandomString(
                            &rnd, rnd.OneIn(20) ? 100 + rnd.Uniform(100)
                                                : rnd.Uniform(8));
