...
//一个可以listen的socket，下面应该就是listen和connect了
        static int get_listen_socket(const std::string &addr, int port) {
            ...
            NOVA_ASSERT(sockfd >= 0) << "ERROR opening listen socket: " << strerror(errno);
            ...
            NOVA_ASSERT(bind(sockfd, (struct sockaddr *) &serv_addr,
        ...
        static int get_send_socket(const std::string &addr, int port, struct timeval timeout = default_timeout) {
            ...
            NOVA_ASSERT((sockfd = socket(AF_INET, SOCK_STREAM, 0)) >= 0) << "Error open socket for send!";
...
//用select方法等待这个端口可以读，超时直接返回
        ...
        static bool wait_recv(int socket, uint32_t timeout = 2000) {
            ...
            while (true) {
                ...
                NOVA_ASSERT(ready != -1) << strerror(errno);
                ...
                if (ready < 0) { // error case
                    NOVA_ASSERT(false) << "select error " << strerror(errno);
                }
