...
// The name of the particular port on the RNIC.
    ...
    struct RNicInfo {
        ...
        bool query_dev_attribute(ibv_context *ctx, ibv_device_attr &attr) {
            ...
            if (rc != 0) {
                NOVA_LOG(ERROR) << "query device attribute error: " << strerror(errno);
                return false;
            }
            return true;
        }
        ...
        void query_port_infos(ibv_context *ctx) {
            ...
            for (uint port_id = 1; port_id <= attr.phys_port_cnt; ++port_id) {
                ...
                if (rc != 0) {
                    NOVA_LOG(ERROR) << "query port_id " << port_id << " on device " << dev_id << "error.";
                    continue;
                }

                // check port status
                if (port_attr.phys_state != IBV_PORT_ACTIVE && port_attr.phys_state != IBV_PORT_ACTIVE_DEFER) {
                    NOVA_LOG(WARNING) << "query port_id " << port_id << " on device " << dev_id << " not active.";
                    continue;
                }
                ...
                switch (port_attr.link_layer) {
                    ...
                    case IBV_LINK_LAYER_INFINIBAND:
                        ...
                        break;
                    default:
                        NOVA_LOG(WARNING) << "unknown link layer at this port: " << port_attr.link_layer;
                        link_layer = "Unknown";
                };
        ...
        void query_active_gids(ibv_context *ctx) {
            ...
            if (rc != 0) {
                NOVA_LOG(WARNING) << "query port attribute at dev " << dev_name << ",port " << port_id
...
//存储了各种关于网卡设备的信息，就是设备的抽象类
    struct RNicHandler {
        ...
        RNicHandler(uint16_t dev_id, uint8_t port_id, ibv_context *ctx, ibv_pd *pd, uint16_t lid, uint16_t gid = 0) :
                ...
                lid(lid),
                gid(gid) {
            NOVA_LOG(DEBUG) << "RNIC " << dev_id << ":" << port_id << ":" << lid << ":" << gid;
        }
