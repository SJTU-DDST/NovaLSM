...
#打印出client server machine的内容，看看都是哪些node
	...
	nova_servers="" # ltc和stoc的服务器加起来的所有server ip:port形式
	nova_all_servers="" # ltc server, ip:port形式
	i="0"
	for s in ${servers[@]}
	do
		nova_port="$port"
...
# 遍历所有machine 删除各个machine的result dir并且重建,清空系统的cache
	...
	do
    	...
    	if [[ $mydebug == "true" ]]; then
			echo "ssh prefix: " "ssh -oStrictHostKeyChecking=no ${ssh_nodes[m]}"
			...
			ssh -oStrictHostKeyChecking=no ${ssh_nodes[m]} "sudo rm -rf $results && mkdir -p $results && chmod -R 777 $results"
			ssh -oStrictHostKeyChecking=no ${ssh_nodes[m]} "sudo sh -c 'echo 3 >/proc/sys/vm/drop_caches'"
		fi
	done
	...
	do
		if [[ $mydebug == "true" ]]; then
			...
			while ssh -oStrictHostKeyChecking=no ${ssh_nodes[m]} "ps -ef | grep -v grep | grep -v ssh | grep -v bash | grep -c \"cp -r\""
			do
				sleep 10
				echo "waiting for $m"
			done
	...
	do
		if [[ $mydebug == "true" ]]; then
			...
			while ssh -oStrictHostKeyChecking=no ${ssh_nodes[m]} "ps -ef | grep -v grep | grep -v ssh | grep -v bash | grep -c nova_server_main"
			do
				sleep 10
				echo "waiting for $m"
			done
	...
	do
		...
		if [[ $mydebug == "true" ]]; then
			...
			ssh -oStrictHostKeyChecking=no ${ssh_nodes[s]} "mkdir -p $cc_stoc_files_path && mkdir -p $db_path && cd $cache_bin_dir && $cmd >& $results/server-$s-out &" &
			server_id=$((server_id+1))
			nova_rdma_port=$((nova_rdma_port+1))
			sleep 1
		fi
	...
	do
		...
		do
			...
			if [[ $mydebug == "true" ]]; then
				...
				ssh -oStrictHostKeyChecking=no ${ssh_nodes[c]} "cd $client_bin_dir && $cmd >& $results/client-$c-$i-out &" &
			fi
		done
	done
	...
	do
		if [[ $mydebug == "true" ]]; then
			...
			while ssh -oStrictHostKeyChecking=no ${ssh_nodes[m]} "ps -ef | grep -v \"grep --color=auto ycsb\" | grep -v ssh | grep -v bash | grep ycsb | grep -c java"
			do
				sleep 10
	...
	do
		...
		if [[ $mydebug == "true" ]]; then
			...
			ssh -oStrictHostKeyChecking=no ${ssh_nodes[s]} "$cmd >& $results/server-$s-db-disk-space"
		fi
	done
	...
	do
		...
		if [[ $mydebug == "true" ]]; then
			...
			ssh -oStrictHostKeyChecking=no ${ssh_nodes[s]} "$cmd >& $results/server-$s-rtable-disk-space"
		fi		
	done
	...
	do
		if [[ $mydebug == "true" ]]; then
			...
			ssh -oStrictHostKeyChecking=no ${ssh_nodes[s]} "mkdir -p $results/server-$server_id-dblogs/ && cp -r $db_path/*/LOG* $results/server-$server_id-dblogs/"
		fi
